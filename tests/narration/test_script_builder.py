...
预期 / Expected:
    21 passed；耗时 < 1s；**全部使用假 provider，零 LLM 调用、零费用**
"""
...
def test_prompt_carries_the_professionalism_rules() -> None:
    ...
    """
    llm = ScriptedProvider("fake", [short_reply("字" * 130)])
...
def test_narration_demands_technical_depth_and_limitations() -> None:
    ...
    """
    llm = ScriptedProvider("fake", [narration_reply("字" * 400)])
...
def test_in_range_result_needs_only_one_call() -> None:
    """达标就结束，不多花一次调用。"""
    llm = ScriptedProvider("fake", [short_reply("字" * 130)])  # 130 字 ≈ 29 秒
    result = build_short_video(article(), llm, low=25, high=35)

    assert llm.call_count == 1
    assert result.calls == 1
    assert result.within_target
    assert 25 <= result.seconds <= 35


def test_overlong_result_triggers_a_rewrite() -> None:
    """超长时回炉，第二稿达标就结束。"""
    llm = ScriptedProvider("fake", [short_reply("字" * 300), short_reply("字" * 130)])
    result = build_short_video(article(), llm, low=25, high=35)
...
def test_rewrite_carries_the_previous_draft_back() -> None:
    ...
    """
    llm = ScriptedProvider("fake", [short_reply("超" * 300), short_reply("字" * 130)])
    build_short_video(article(), llm, low=25, high=35)
...
def test_short_video_returns_title_and_subtitle() -> None:
    ...
    llm = ScriptedProvider(
        "fake", [short_reply("字" * 130, title="推理成本砍半", subtitle="吞吐提升 2.3 倍")]
    )
...
def test_narration_targets_the_longer_window() -> None:
    ...
    llm = ScriptedProvider("fake", [narration_reply("字" * 400)])  # ≈ 89 秒
    result = build_narration(article(), llm, low=60, high=120)

    assert llm.call_count == 1
    assert result.within_target
    assert 60 <= result.seconds <= 120
...
def test_narration_also_returns_a_title_pair() -> None:
    ...
    """
    llm = ScriptedProvider(
        "fake", [narration_reply("字" * 400, title="推理引擎开源", subtitle="吞吐提升 2.3 倍")]
    )
...
def test_rewrite_delta_is_computed_from_the_draft_own_density() -> None:
    """
    回炉的字数差值按**上一稿实测的字符密度**换算，不是预设的 4.5 字/秒。

    一篇塞满英文标识符的稿子实测能到 8 字符/秒。按 4.5 算，超时 10 秒只会要求
    删 45 字，而实际需要删 80 字——第二稿仍然超时，白花一次调用。
    这里用纯英文稿构造高密度：字符多、时长短，两种换算的差值明显不同。
    """
    # 纯英文词：字符密度远高于中文，正好放大两种换算的差距
    english = "Terminal Bench throughput benchmark result " * 40
    llm = ScriptedProvider("fake", [short_reply(english), short_reply("字" * 130)])
    build_short_video(article(), llm, low=25, high=35)

    seconds = estimate_seconds(english)
    overrun = seconds - 35
    naive = int(overrun * CHARS_PER_SECOND_ZH)
    calibrated = int(overrun * observed_chars_per_second(len(english), seconds))

    feedback = joined(llm.messages[1])
    assert "删掉约" in feedback
    assert calibrated > naive * 2, "英文稿的字符密度远高于 4.5，差值必须跟着放大"
    assert f"删掉约 {calibrated}" in feedback
