...
预期 / Expected:
    耗时 < 2s；只在 pytest 的 tmp_path 下建库，不碰真实 data/
"""
...
def test_count_matches_list_under_the_same_filters(ledger: Ledger) -> None:
    """
    `count()` 与 `list()` 在同一组筛选下必须数一样多 —— 翻页全靠这一点。

    两边各拼一份 WHERE 的话，总页数与页内容会悄悄错开，而这种错看起来
    只是「最后一页是空的」，不像 bug。所以它们共用 `_where()`，这条测试守住它。
    """
    ok_id, _ = ledger.register(raw("https://e.com/1", title="多模态大模型发布"))
    ledger.record_fetch(ok_id, article())
    ledger.register(raw("https://e.com/2", title="芯片行业动态", source="infoq-cn"))
    ledger.register(raw("https://other.com/3", title="多模态推理加速"))
    ...
    for filters in (
        ...
        assert ledger.count(**filters) == len(ledger.list(limit=100, **filters)), filters


def test_count_ignores_limit_and_offset(ledger: Ledger) -> None:
    """总数是「满足筛选的全部」，不受当前这一页影响。"""
    for i in range(7):
        ledger.register(raw(f"https://e.com/{i}"))

    assert ledger.count() == 7
    assert len(ledger.list(limit=3, offset=6)) == 1


def test_offset_pages_through_without_repeating(ledger: Ledger) -> None:
    """逐页取完不重不漏 —— 翻页看到的是同一批文章切开，不是各取各的。"""
    for i in range(5):
        ledger.register(raw(f"https://e.com/{i}"))

    page1 = [r.id for r in ledger.list(limit=2, offset=0)]
    page2 = [r.id for r in ledger.list(limit=2, offset=2)]
    page3 = [r.id for r in ledger.list(limit=2, offset=4)]
