...
五项信号各占多少权重、为什么用规则而不用 LLM 打分，见
[04_architecture_src.md](04_architecture_src.md) 的 `pipeline/score.py` 一节
（本文档不复述数值，抄一份出来只会各自漂移）。
...
**期次目录里没有条目资产**，按 id 引用过去。完整规范见
[05_output_spec.md](05_output_spec.md)（落盘唯一权威）。
...
**相同的请求会命中本地磁盘缓存，不重复计费**，同样的输入再跑一遍是零调用。
命令结尾会打印命中率：
...
缓存在 `data/llm_cache/`，删掉即可全部重来；不想用缓存时加 `--no-cache`。
缓存键的构成与「重做为什么必须绕开缓存」见
...
**没有主线综述** —— 条目太少时不生成（阈值与理由见
[06_prompt_spec.md](06_prompt_spec.md)）。

**去重合并错了** —— `--dry-run` 里能看到「近重复合并 N」。阈值取的是保守值：
宁可漏合并（你自己看得出两条相似），也不要错合并（两件事混成一条，读者无从察觉）。
